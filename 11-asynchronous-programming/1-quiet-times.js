...
has set up a system that notices this kind of wireless network traffic
...
function activityTable(day) that for a given day of the week returns an
array of 24 numbers, one for each hour of the day, that hold the number
of camera network traffic observations seen in that hour of the day. Days
are identified by number using the system used by Date.getDay, where
Sunday is 0 and Saturday is 6.
The activityGraph function, provided by the sandbox, summarizes
such a table into a string.
To read the files, use the textFile function defined earlier—given a
filename, it returns a promise that resolves to the file’s content. Re-
member that new Date(timestamp) creates a Date object for that time,
which has getDay and getHours methods returning the day of the week
...
async function activityTable(day) {
  ...
  for (const filename of logFileList.split('\n')) {
